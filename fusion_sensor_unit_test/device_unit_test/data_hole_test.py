...
class DataHolekTestCase(unittest.TestCase):
    ...
    def testDataHole(self):
        ...
        for placementId in placement_id_list:
            ...
            vdatalist = collections.OrderedDict(
                sorted(
                    self.parseVideoFileList(
                        vfilelist,
                        start_datetime,
                        end_datetime).items()))
            wdatalist = collections.OrderedDict(
                sorted(
                    self.parseWifiFileList(
                        wfilelist,
                        start_datetime,
                        end_datetime).items()))
            # print vdatalist
        ...
        #            wdata_show[wdata_items[i][0]] = wdata_items[i][1]
            ...
            if vdatalist:
                ...
                vend = datetime.datetime.strptime(
                    vdatalist.keys()[len(vdatalist.keys()) - 1], '%m %d %H:%M:%S')
                velaps = vend - vstart
                vtotal_run = divmod(velaps.total_seconds(), 86400)
                vtotal_h = divmod(vtotal_run[1], 3600)
                vtotal_m = divmod(vtotal_h[1], 60)
                vtotal_run_str = "%d days %d hours %d mins" % (
                    vtotal_run[0], vtotal_h[0], vtotal_m[0])

            wtotal_run_str = ""
            if wdatalist:
                wstart = datetime.datetime.strptime(wdatalist.keys()[0], '%m %d %H:%M:%S')
                wend = datetime.datetime.strptime(
                    wdatalist.keys()[len(wdatalist.keys()) - 1], '%m %d %H:%M:%S')
                welaps = wend - wstart
                wtotal_run = divmod(welaps.total_seconds(), 86400)
                wtotal_h = divmod(wtotal_run[1], 3600)
                wtotal_m = divmod(wtotal_h[1], 60)
                wtotal_run_str = "%d days %d hours %d mins" % (
                    wtotal_run[0], wtotal_h[0], wtotal_m[0])
