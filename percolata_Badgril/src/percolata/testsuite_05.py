...
#创建数据文件
case5file = open(db2file.case3datafile)
...
class TestCase_01(unittest.TestCase):
    ...
    #从数据文件获取保单号
    pn = Autotest.dataRead(db2file.case5datafile)  #读取数据文件中的保单号 
    mn = '' #现金价值
    @classmethod
    ...
    def test_04_xxlr(self):
        ...
        #服务项目
        driver.find_element_by_id('applyFiles0.serviceItems').send_keys('37')
        #申请日志为今天
    ...
    def test_07_xxlr(self):
        ...
        #输入领取现金价值
        wa = driver.find_element_by_xpath('/html/body/div[2]/div[2]/form/div[3]/div/div[2]/div/table/tbody/tr/td[1]').text
        money = string.atof(wa)
        if money >= 500 :
            driver.find_element_by_id('withdrawAmount').send_keys('500')
        else:
            driver.find_element_by_id('withdrawAmount').send_keys(wa)
    ...
    def test_08_jbjg(self):
        ...
        try:
            self.assertIn(mn, mn1, '处理规则检查不通过！')
        except AssertionError,e:
            print e
            #获取当前时间
            now = time.strftime("%Y-%m-%d_%H_%M_%S")
            print ' 请查看截图文件 '+now+'.png'
            self.driver.get_screenshot_as_file(xmlpath2+"log\\"+now+U'现金价值计算错误或处理规则检查不通过.png')#如果没有找到上面的元素就截取当前页面。
        #断言:经办结果页面正常
        try:
