...
#创建数据文件
case2file = open(db2file.case2datafile)
...
class TestCase_01(unittest.TestCase):
    ...
    #从数据文件获取保单号
    pn = Autotest.dataRead(db2file.case2datafile)  #读取数据文件中的保单号 
    mn = '' #现金价值
    @classmethod
    ...
    def test_04_xxlr(self):
        ...
        #服务项目
        driver.find_element_by_id('applyFiles0.serviceItems').send_keys('2')
        #申请日志为今天
    ...
    def test_05_bqqr(self):
        ...
        try:
            ...
            self.assertEqual(srt, U'通过', U'受理规则检查不通过')
    ...
    def test_07_xxlr(self):
        ...
        #勾选保全保单
        driver.find_element_by_name('productCb0').click()
        #点击确定
    ...
    def test_08_jbjg(self):
        ...
        try:
            self.assertIn(mn, mn1, '现金价值计算错误或处理规则检查不通过！')
        except AssertionError,e:
            print e
            print ' 请查看截图文件 '+now+'.png'
        ...
        #断言:经办结果页面正常
        try:
            self.assertEqual(u"机构受理>>逐单受理>>受理经办结果", self.driver.find_element_by_id("pathString").text,U'受理经办页面显示异常！')
        except AssertionError,e:
            print e
            print ' 请查看截图文件 '+now+'.png'
    ...
    def tearDownClass(cls):
        Autotest.dataDelline(db2file.case2datafile) #删除一行数据文件
        cls.driver.quit()
