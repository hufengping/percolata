...
def get_curlRate(conf,devName):
    ...
    startTime = time.time()#####
    ssh_curl_proc.sendline(r"curl -k -H 'Expect:' -F 'file=@%s_BW_Calc' -i %svideo_file?id=%s\&filename=%s_BW_Calc\&md5=%s"%(devName,conf['uploadURL'],devName,devName,md5Value))
    try:
        wait_server(conf,ssh_curl_proc,'node',3600)
    except Exception as err:
        if('TIMEOUT'==err.message):
            pexpect.run("bash -c \"echo '===%s===: upload rate beneath lower limit' >> BW_record\""%devName)
        else:
...
if __name__ == '__main__':
    ...
    pexpect.run("bash -c \"echo '===%s=== curlRate: %s KBps  |  dataRate: %sKBps' >> BW_record\""%(devName,curlRate,dataRate))
