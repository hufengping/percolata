...
class VideopUnwarper(object):
    ...
    def _get_image_affine_map(self, transformed_image_size, inv_homo_mat):
        ...
        for row in xrange(transformed_image_size.height):
            for col in xrange(transformed_image_size.width):
                image_affine_map[row, col, 0] = transformed_pixel_locations[0, pixel_idx] / transformed_pixel_locations[2, pixel_idx]
                image_affine_map[row, col, 1] = transformed_pixel_locations[1, pixel_idx] / transformed_pixel_locations[2, pixel_idx]
                pixel_idx += 1
...
class VideoPreviewer(object):
    ...
    def preview(self):
        ...
        print uncropped_unwarp_config
        loiParam = None
        if uncropped_unwarp_config.has_key('tripline'):
            loiParam = uncropped_unwarp_config['tripline']
        ...
        left_x = new_unwarp_params["roiX"] * frame_size.width
        ...
        while True:
            ...
            cv2.line(rectified_frame,
                    ...
                    frame_right_bottom.to_tuple(int), (255,255,0), 2)
            cv2.line(rectified_frame,
